...
The `Elody base graphql` already contains [Datasources](https://www.apollographql.com/docs/apollo-server/data/fetching-data) to interact whith the default Elody backend services. Custom `Datasources` can be created in the `client specific graphql module` repositories or your `custom elody modules`.
...
The composables live in our `composables` folder in the frontend and their name alsways starts with _use_, for example `useBaseLibrary` to keep the state and functionality for our `baseLibrary` component.
